...
L_values = [0.1, 0.15, 0.23];  % You can adjust these values as needed

rpm = 26280 * (2*pi/60); % max rpm output of each motor
...
%% Final form of equation conversion from symbols to numerics 
    ...
    w_dot_str=strrep(w_dot_str,'W1(t)','650');
    w_dot_str=strrep(w_dot_str,'W2(t)','660');
    w_dot_str=strrep(w_dot_str,'W3(t)','650');
    w_dot_str=strrep(w_dot_str,'W4(t)','660');
...
% Dis_G_values = cell(length(L_values), 1);
...
for i = 1:length(L_values)
    ...
    for j = 1:length(t)
        ...
        Acc_G=strrep(Acc_G,'W1(t)','650');
        Acc_G=strrep(Acc_G,'W2(t)','660');
        Acc_G=strrep(Acc_G,'W3(t)','650');
        Acc_G=strrep(Acc_G,'W4(t)','660');
